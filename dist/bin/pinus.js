...
function start(opts) {
    ...
    if (opts.daemon) {
        ls = child_process_2.spawn(process.execPath, params, { detached: true, stdio: 'ignore' });
        ls.unref();
        console.log(DAEMON_INFO);
        process.exit(0);
    }
    else {
        ls = child_process_2.spawn(process.execPath, params);
...
function terminal(signal, opts) {
    ...
    if (opts.force) {
        if (os.platform() === constants.PLATFORM.WIN) {
            child_process_1.exec(KILL_CMD_WIN);
        }
        else {
            child_process_1.exec(KILL_CMD_LUX);
        }
        process.exit(1);
        return;
    }
